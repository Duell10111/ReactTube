...
func saveVideoResponse(_ session: WCSession, message: [String: Any]) {
  if let id = message["id"] as? String, let title = message["title"] as? String, let duration = message["duration"] as? Int, let streamURL = message["steamURL"] as? String, let validUntil = message["validUntil"] as? Int, let coverURL = message["coverUrl"] as? String {
